...
/**
 * Keywords used to identify different sections in a resume.
 */
const SECTION_KEYWORDS = [
  'work', 'experience', 'employment', 'history', 'job', 
  'expériences professionnelles', 'expérience', 'travail', 'emploi',
  'education', 'formation', 'éducation', 'études', 'skills', 'compétences'
];

/**
 * Filters lines that match any of the provided keywords.
 * 
 * @param sections - The entire document lines.
 * @param keywords - Keywords to identify the section.
 * @returns Filtered lines belonging to the section.
 */
export const getSectionLinesByKeywords = (
  sections: Record<string, Lines>,
  keywords: string[]
): Lines => {
  const lowercasedKeywords = keywords.map(keyword => keyword.toLowerCase());

  for (const [sectionName, lines] of Object.entries(sections)) {
    if (lowercasedKeywords.some(keyword => sectionName.toLowerCase().includes(keyword))) {
      return lines;
    }
  }
...
/**
 * Group text items into lines. This returns an array where each position

...
const SEASONS = ["Summer", "Fall", "Spring", "Winter", "Été", "Automne", "Printemps", "Hiver"];
const hasSeason = (item: TextItem) =>
  SEASONS.some((season) => item.text.includes(season));
const hasPresent = (item: TextItem) => item.text.includes("Present") || item.text.includes("Présent");
